...
sArgParser = argparse.ArgumentParser(
    add_help=False,
    description="""Use your favorite search engine to search for a search term with different websites. Use single quotes around a query with double quotes. Be sure to enclose a query with single quotes it contains shell control characters like space, ';', '>', '|', etc.""",
)
sArgParser.add_argument(
    "-h",
    "--help",
    help="Show this help message, print categories on file (add -file to check other CSV file) and exit.",
    action="store_true",
)
sArgParser.add_argument(
    "-hh",
    "--help2",
    help="Show the help inside a .csv file being called. Lines in the beginning of the script starting with # are displayed as help.",
    action="store_true",
)
sArgParser.add_argument(
    "-browser",
    metavar="<browser>",
    help="Supply the browser executable to use or use the default browser.",
    default=None,
)
sArgParser.add_argument(
    "-cat",
    metavar="<category>",
    help="Choose from 1 or more categories, use ',' (comma) as delimiter. Defaults to all categories.",
)
sArgParser.add_argument(
    "-cats", help="Show all categories on file, use with or without -file.", action="store_true"
)
sArgParser.add_argument(
    "-count",
    metavar="<count>",
    help="How many websites are searched per query. Google has a maximum length for queries.",
)
sArgParser.add_argument(
    "-engine",
    metavar="<engine>",
    help="Search with 'google', 'baidu', 'bing', 'bing-ecosia', 'duckduckgo' 'yahoo' or 'yandex', defaults to 'google'.",
    choices=["bing", "bing-ecosia", "baidu", "duckduckgo", "google", "yahoo", "yandex"],
    default="google",
)
sArgParser.add_argument("-file", metavar="<file>", help="Enter a custom website list.")
sArgParser.add_argument(
    "-filter", metavar="<string>", help="Only query for sites with this string."
)
sArgParser.add_argument("-query", metavar="<query>", help="Enter a mandatory search term.")
sArgParser.add_argument(
    "-site",
    metavar="<on|off|inurl>",
    help="Turn the 'site:' operator 'on' or 'off', or replace it with 'inurl:' (only for Google), defaults to 'on'.",
    default="on",
    choices=["on", "off", "inurl"],
)
sArgParser.add_argument(
    "-excl", metavar="<domains>", help="Excluded these domains from the search query."
)
sArgParser.add_argument(
    "-echo",
    help="Prints the search query URLs, for further use like piping or bookmarking.",
    action="store_true",
)
sArgParser.add_argument(
    "-ubb",
    help="Updates bug bounty files (in en out scope) and exits. Uses bbrecon.",
    action="store_true",
)
sArgParser.add_argument(
    "-wait", metavar="<seconds>", help="Wait x seconds, defaults to 7 seconds.", default=7
)
...
if aArguments.ubb:
    ...
    sOutput = subprocess.check_output(["bbrecon", "get", "programs", "--type", "web", "-o", "json"], timeout=60)
    fCsvInScope = open(
    ...
    fCsvOutScope = open(
        os.path.dirname(os.path.realpath(sys.argv[0])) + "/sitedorks-bbrecon-outscope.csv",
        "w",
        buffering=1,
    )
            ...
            if sOutScope["type"] == "web":
                ...
                for sOutScopeValue in lOutScopeValues:
                    ...
                    if (
                        ...
                        if sDomain not in dDomainsInScope:
...
if not aArguments.cat and aArguments.query and not aArguments.help:
    while sAnswer.lower() != "y" and sAnswer.lower() != "n":
        sAnswer = input(
            "Not providing -cat can open a whole lot of tabs/windows in your browser. Do you want to continue? (y/n) "
        )
...
if (
    aArguments.site == "inurl"
    and aArguments.engine != "google"
    and aArguments.engine != "duckduckgo"
):
