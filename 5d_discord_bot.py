...
async def project_plan(ctx):
    ...
    embed = discord.Embed(title="🚀 5D Action Plan", color=discord.Color.gold())
...
async def research_updates(ctx, keyword="self-directed learning"):
    ...
    embed = discord.Embed(title=f"📚 Research: {keyword}", color=discord.Color.purple())
...
async def github_repos(ctx, query="education"):
    ...
    embed = discord.Embed(title=f"💻 GitHub: {query}", color=discord.Color.dark_blue())
