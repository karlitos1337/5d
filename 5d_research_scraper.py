...
class ResearchScraper:
    ...
    def _rate_limit(self, domain="default", delay=None):
        """
        Enforce rate limiting between requests for a specific domain.

        Args:
            domain: The domain key for the lock
            delay: Optional override for rate limit delay (seconds)
        """
        limit = delay if delay is not None else self.rate_limit_delay
    ...
    def _fetch_single_wb_indicator(self, indicator_code, indicator_name, countries_str):
        """Helper to fetch a single World Bank indicator."""
        # Use reduced delay (0.2s) for parallel requests to allow higher throughput
        self._rate_limit("worldbank", delay=0.2)
    ...
    def fetch_world_bank_education_data(self, countries=None):
        ...
        print(f"  🏫 World Bank: Fetching {len(indicators)} indicators in parallel...")

        with ThreadPoolExecutor(max_workers=4) as executor:
            future_to_indicator = {
                executor.submit(self._fetch_single_wb_indicator, code, name, countries_str): name
            ...
            for future in as_completed(future_to_indicator):
                ...
                for item in results:
                    ...
                    if country_code and item["value"] is not None:
                        if country_code not in education_data:
                            education_data[country_code] = {}

                        # Keep most recent data (assuming logic from original: if not exists)
                        # Original: if indicator_name not in education_data[country_code]:
                        if name not in education_data[country_code]:
                            education_data[country_code][name] = {
                                "value": item["value"],
                                "year": item["year"]
                            }
