...
class ResearchScraper:
    ...
    def fetch_world_bank_wgi_data(self, countries=None):
        """
        Fetch Voice & Accountability (VA.EST) from World Bank WGI (Source 3).
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # World Bank API endpoint
                # We fetch recent data. WGI is annual.
                countries_str = ";".join(countries[:10])  # Limit to 10 per request
                url = f"{self.wb_base_url}/country/{countries_str}/indicator/{indicator_code}"
                params = {"format": "json", "date": "2020:2023", "source": 3, "per_page": 500}
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.rate_limit_delay * (self.retry_backoff**attempt)
                    print(
                        f"    ⚠️  World Bank WGI error (attempt {attempt + 1}/{self.max_retries}): {e}"
                    )
                    time.sleep(wait_time)
                else:
    ...
    def scrape_all(self):
        ...
        all_research["world_bank_wgi"] = {
            "data": wgi_data,
            "timestamp": datetime.now().isoformat(),
            "source": "World Bank WGI (Source 3)",
        }
