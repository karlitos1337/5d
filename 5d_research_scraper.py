...
class ResearchScraper:
    def __init__(self, rate_limit_delay=1.0, max_retries=3, retry_backoff=2.0):
        ...
        self.locks = {}
        self._get_lock_lock = threading.Lock() # Lock for accessing self.locks
    ...
    def _process_keyword(self, keyword):
        ...
        # Execute searches concurrently for this keyword
    ...
    def scrape_all(self):
        ...
        # 1. Fetch Academic Papers (Parallelized)
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            # Create a future for each keyword
            future_to_keyword = {
                executor.submit(self._process_keyword, keyword): keyword
        ...
        # 2. Fetch Global Data (Concurrent execution of WHO and World Bank)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            ...
            future_who = executor.submit(lambda: ("who_mental_health", {
                "data": {}, # self.fetch_who_mental_health_data(),
                "timestamp": datetime.now().isoformat(),
                "source": "WHO Global Health Observatory (Disabled)"
            }))

            # World Bank Education Data
            future_wb = executor.submit(lambda: ("world_bank_education", {
                "data": self.fetch_world_bank_education_data(),
                "timestamp": datetime.now().isoformat(),
                "source": "World Bank EdStats API"
            }))

            for future in concurrent.futures.as_completed([future_who, future_wb]):
                key, data = future.result()
                all_research[key] = data
                if key == "who_mental_health":
                    print("\n🏥 Fetching WHO Mental Health Data (SKIPPED - TODO: Fix API)...")
                elif key == "world_bank_education":
...
if __name__ == "__main__":
    ...
    # Statistik
    total_papers = sum(len(data.get("arxiv", [])) + len(data.get("pubmed", [])) for data in research_data.values() if isinstance(data, dict))
