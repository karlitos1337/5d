...
class ResearchScraper:
    ...
    def fetch_world_bank_data(self, countries=None):
        ...
        Returns:
            dict: World Bank data by country
        """
        if countries is None:
            countries = [
                "USA",
        ...
        for indicator_code, indicator_name in indicators.items():
            print(f"  🏫 World Bank: Fetching {indicator_name} ({indicator_code})...")
                    ...
                    if isinstance(data, list) and len(data) > 1:
                        for entry in data[1]:  # Data is in second element
                            ...
                            if country_code and value is not None:
                                if country_code not in wb_data:
                                    wb_data[country_code] = {}

                                # Keep most recent data
                                if indicator_name not in wb_data[country_code]:
                                    wb_data[country_code][indicator_name] = {
                                        "value": value,
                                        "year": year,
                                    }
        ...
        print(f"  ✅ World Bank: {len(wb_data)} countries fetched")
        return wb_data
    ...
    def scrape_all(self):
        ...
        print("\n🏫 Fetching World Bank Data (Education + Governance)...")
        wb_data = self.fetch_world_bank_data()
        all_research["world_bank_data"] = {
            "data": wb_data,
            "timestamp": datetime.now().isoformat(),
            "source": "World Bank API (EdStats + WGI)",
        }
