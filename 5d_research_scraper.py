...
class ResearchScraper:
    ...
    def fetch_world_bank_wgi_data(self, countries=None):
        """
        Fetch Governance Indicators (WGI) from World Bank API.
        ...
        for indicator_code, indicator_name in indicators.items():
            ...
            # Chunk countries into batches of 10
            for i in range(0, len(countries), 10):
                batch = countries[i:i+10]
                ...
                for attempt in range(self.max_retries):
                    try:
                        ...
                        # Some indicators might return 400 or 404 for certain countries/years, just warn and continue
                        if response.status_code != 200:
                            if attempt < self.max_retries - 1 and response.status_code >= 500:
                                pass # retry on server errors
                            else:
                                print(f"    ⚠️  World Bank API returned {response.status_code} for batch {countries_str}")
                                break
                        ...
                        try:
                            ...
                            print("    ⚠️  World Bank API did not return JSON")
                            break
                        ...
                        if isinstance(data, list) and len(data) > 1:
                            for entry in data[1]:  # Data is in second element
                                ...
                                if country_code and value is not None:
                                    ...
                                    # Keep most recent data (API returns descending by year usually, but we take first valid)
