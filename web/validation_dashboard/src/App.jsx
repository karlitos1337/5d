...
const App = () => {
  ...
  useEffect(() => {
    // ⚡ Bolt: Throttling scroll events using a ticking flag to prevent excessive calls.
    let ticking = false;

    const handleScroll = () => {
      if (!ticking) {
        // ⚡ Bolt: Using requestAnimationFrame to ensure calculations run exactly before the next paint.
    ...
    // ⚡ Bolt: Adding `{ passive: true }` improves scroll performance by telling the browser
    // that the listener will not call preventDefault(), preventing main-thread blocking.
