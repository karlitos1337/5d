...
export async function fetchWithCache(key, fetcher) {
  ...
  try {
    ...
    // Re-load cache to get latest state before writing (prevents race conditions)
...
export async function fetchAllData() {
  ...
  // --- Deduplication & Shared Logic ---

  // Shared Promise for Depression Data (CSV)
  let _depressionRowsPromise = null;
  const getDepressionRows = () => {
    if (!_depressionRowsPromise) {
      _depressionRowsPromise = (async () => {
    ...
    return _depressionRowsPromise;
  };

  // Shared Promise for Dropout Data (JSON)
  let _dropoutRowsPromise = null;
  const getDropoutRows = () => {
    if (!_dropoutRowsPromise) {
      _dropoutRowsPromise = (async () => {
        try {
    ...
    return _dropoutRowsPromise;
  };

  const buildSeries = (rows, codeKey = 'Code', yearKey = 'Year', valKey = null) => {
    const series = {};
    for (const r of rows) {
      const code = r[codeKey];
      const year = Number(r[yearKey]);
      const actualValKey = valKey || (r ? Object.keys(r).slice(-1)[0] : 'Value');
      const val = r[actualValKey];
  ...
  // --- Define Promises (Parallel Execution) ---

  const pSchools = fetchWithCache('schools', () => fetchJSON('./data/schools.json')).catch(() => []);
  const pCountries = fetchWithCache('countries', () => fetchJSON('./data/countries.json')).catch(() => []);
  const pValidation = fetchWithCache('validation', () => fetchJSON('./data/validation.json')).catch(() => ({ validatedISO3: [], items: [] }));
  const pBaseline = fetchWithCache('baseline_snapshot', () => fetchJSON('./data/baseline.json')).catch(() => null);

  const pDepressionMap = fetchWithCache('owid_depression', async () => {
    const rows = await getDepressionRows();
    return reduceLatestByCode(rows, 'Code');
  }).catch(() => ({}));

  const pDepressionSeries = fetchWithCache('owid_depression_series', async () => {
    const rows = await getDepressionRows();
    // Depression CSV: Year is "Year", Code is "Code"
    return buildSeries(rows, 'Code', 'Year');
  }).catch(() => ({}));

  const pDropoutMap = fetchWithCache('wb_dropout', async () => {
    const rows = await getDropoutRows();
    // Dropout WB JSON: countryiso3code, date (year), value
    const latest = {};
    for (const r of rows) {
  ...
  const pDropoutSeries = fetchWithCache('wb_dropout_series', async () => {
    const rows = await getDropoutRows();
    return buildSeries(rows, 'countryiso3code', 'date', 'value');
  }).catch(() => ({}));
  ...
  const pWgiRl = fetchWithCache('wgi_rl_est', () => wgiFetch('RL.EST')).catch(() => ({}));
  const pWgiVa = fetchWithCache('wgi_va_est', () => wgiFetch('VA.EST')).catch(() => ({}));
  const pWgiGe = fetchWithCache('wgi_ge_est', () => wgiFetch('GE.EST')).catch(() => ({}));

  const pWorldGeoJSON = fetchWithCache('world_geojson', async () => {
  ...
  // --- Await All ---
  const [
    schools, countries, validation, baseline,
    depressionMap, depressionSeries,
    dropoutMap, dropoutSeries,
    wgi_rl_raw, wgi_va_raw, wgi_ge_raw,
    worldGeoJSON
  ] = await Promise.all([
    pSchools, pCountries, pValidation, pBaseline,
    pDepressionMap, pDepressionSeries,
    pDropoutMap, pDropoutSeries,
    pWgiRl, pWgiVa, pWgiGe,
    pWorldGeoJSON
  ]);
