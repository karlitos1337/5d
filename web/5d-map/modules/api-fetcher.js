...
// Helper to clear legacy monolithic cache if it exists
function clearLegacyCache() {
  try {
    localStorage.removeItem('5d-map-cache-v1');
  } catch {
    // ignore
  }
}
...
export function clearCache() {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('5d-map-v1:')) {
...
async function fetchWithCache(key, fetcher) {
  const storageKey = `5d-map-v1:${key}`;
  const now = Date.now();
...
export async function fetchAllData() {
  ...
  const [
    ...
    fetchWithCache('owid_depression_series', async () => {
      ...
      try {
        ...
        console.warn('Depression series remote fetch failed, using local fallback:', e.message);
        const localRes = await fetch('./data/depression_sample.csv');
        const localText = await localRes.text();
        const localRows = parseCSV(localText);
    ...
    fetchWithCache('wb_dropout', async () => {
      ...
      for (const r of rows) {
        const iso3 = r?.countryiso3code; const year = Number(r?.date);
        const val = r?.value == null ? null : Number(r.value);
        if (!iso3 || val == null || Number.isNaN(val)) continue; // !year check is intentionally loose here to match original
        const prev = latest[iso3];
        if (!prev || year > prev.year) latest[iso3] = { value: val, year };
      }
      const map = {};
  ...
  result.worldGeoJSON = worldGeoJSON; // Assign fetched GeoJSON
