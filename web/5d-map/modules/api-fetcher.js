...
async function fetchWithCache(key, fetcher) {
  ...
  try {
    ...
    // Re-read cache immediately before writing to prevent race conditions during concurrent execution
    const freshCache = loadCache();
    freshCache[key] = { data, timestamp: Date.now() };
...
export async function fetchAllData() {
  ...
  // Parallelize independent fetch requests using Promise.all to reduce total wait time
  const [
    schoolsData,
    countries,
    validation,
