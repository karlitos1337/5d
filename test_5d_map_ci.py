        ...
        for country_code, country_data in countries.items():
            for field in required_fields:
                self.assertIn(field, country_data,
                            f"Country {country_code} missing {field}")
...
class FormulaCalculationTests(unittest.TestCase):
    ...
    def test_imp_score_formula(self):
        ...
        result = self.calculate_imp_score(gov_score=60, depression_rate=15,
                                         dropout_rate=10, school_bonus=10)
...
class PerformanceBenchmarkTests(unittest.TestCase):
    ...
    def test_data_loading_performance(self):
        ...
        self.assertLess(load_time, 1.0,
                       f"Data loading took {load_time}s (should be < 1s)")
