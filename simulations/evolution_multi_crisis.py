...
def run_multi_crisis_experiment(
    ...
    print(f"\n{'='*60}")
    print(f"🧬 EVOLUTION MULTI-CRISIS EXPERIMENT")
    print(f"{'='*60}")
    ...
    for i, crisis_type in enumerate(crisis_types):
        ...
        print(f"  Free: {survivors_free} survivors ({stats_free['survival_rate']*100:.1f}%), diversity={stats_free['diversity']:.2f}")
        print(f"  Controlled: {survivors_controlled} survivors ({stats_controlled['survival_rate']*100:.1f}%), diversity={stats_controlled['diversity']:.2f}")
        print()
...
def visualize_results(results, pop_free, pop_controlled, save_path=None):
    ...
    ax.plot(crisis_range, free_survival, 'o-', label='Free (High Diversity)', linewidth=2, markersize=8, color='#10b981')
    ax.plot(crisis_range, controlled_survival, 's-', label='Controlled (Low Diversity)', linewidth=2, markersize=8, color='#ef4444')
    ...
    ax.plot(crisis_range, controlled_diversity, 's-', label='Controlled', linewidth=2, markersize=8, color='#ef4444')
    ax.set_xlabel('Crisis Number', fontsize=12)
    ...
    ax.set_xlabel('Trait Value', fontsize=12)
    ax.set_ylabel('Count', fontsize=12)
    ax.set_title(f'Controlled Population Traits (n={len(alive_traits_controlled)})', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='y')
    ...
    ax.bar(crisis_counts.keys(), crisis_counts.values(), 
...
if __name__ == '__main__':
    ...
    print(f"📊 Hypothesis Test: Does diversity help over multiple crises?")
