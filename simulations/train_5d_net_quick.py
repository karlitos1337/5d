...
from five_d_net import FiveDNet, BaselineNet
...
def train_quick(model: nn.Module, loader: DataLoader, optimizer: optim.Optimizer,
                config: QuickConfig) -> Dict[str, float]:
...
def evaluate_quick(model: nn.Module, loader: DataLoader, config: QuickConfig,
...
def main():
    ...
    for model_name in ["5D-Net", "Baseline"]:
        ...
        if model_name == "5D-Net":
            model = FiveDNet(input_size=784, hidden_size=config.hidden_dim, 
                            num_classes=10).to(config.device)
        else:
            model = BaselineNet(input_size=784, hidden_size=config.hidden_dim, 
                               num_classes=10).to(config.device)
    ...
    print(f"  Expected: 5D -12%, Baseline -28%")
