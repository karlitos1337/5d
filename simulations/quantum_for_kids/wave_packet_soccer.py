...
class QuantumSoccerBall:
    ...
    def _create_wave_packet(self):
        ...
        gaussian = np.exp(-((self.X - self.x0)**2 / (2*self.sigma_x**2) +
...
class SoccerWaveVisualizer:
    ...
    def setup_plot(self):
        ...
        self.fig.suptitle('Quantum Wave Packets via FUSSBALL! ⚽🌊', 
                         fontsize=16, fontweight='bold')
        
        # Left: Before measurement (wave packet)
        self.ax1.set_title('BEFORE: Ball als Welle (überall gleichzeitig!)', 
        ...
        self.ax2.set_title('AFTER: Torwart fängt → Welle kollabiert!', 
                          fontsize=12, fontweight='bold')
    ...
    def draw_field(self, ax):
        ...
        # Goal
        goal = patches.Rectangle((4, 0), 2, 0.5,
                                linewidth=2, edgecolor='white',
                                facecolor='white', alpha=0.5)
        ax.add_patch(goal)
        
        # Center circle
        circle = patches.Circle((5, 5), 1.5,
                               linewidth=2, edgecolor='white',
    ...
    def visualize_step(self, goalkeeper_x=5.0, goalkeeper_y=1.0, num_frames=5):
        ...
        for frame in range(num_frames):
            ...
            im = self.ax1.contourf(self.ball.X, self.ball.Y, self.ball.wave_packet,
            ...
            max_idx = np.unravel_index(self.ball.wave_packet.argmax(), 
                                      self.ball.wave_packet.shape)
            peak_x = self.ball.x[max_idx[1]]
            peak_y = self.ball.y[max_idx[0]]
            self.ax1.plot(peak_x, peak_y, 'o', markersize=20, 
                         color='white', markeredgecolor='black', 
                         markeredgewidth=2)
            self.ax1.text(peak_x, peak_y, '⚽', ha='center', va='center',
                         fontsize=20)
            
            # Add goalkeeper
            self.ax1.plot(goalkeeper_x, goalkeeper_y, 's', markersize=25,
                         color='blue', markeredgecolor='black', markeredgewidth=2)
            self.ax1.text(goalkeeper_x, goalkeeper_y, '🧤', ha='center', va='center',
                         fontsize=20)
            
            # Annotations
            self.ax1.text(5, 9.5, f'Frame {frame+1}/{num_frames}: Welle breitet aus!',
        ...
        im2 = self.ax2.contourf(self.ball.X, self.ball.Y, self.ball.wave_packet,
                               levels=20, cmap='Blues', alpha=0.7)
        plt.colorbar(im2, ax=self.ax2, label='Collapsed State')
        
        # Ball at measured position
        self.ax2.plot(measured_pos[0], measured_pos[1], 'o', markersize=30,
                     color='white', markeredgecolor='black', markeredgewidth=3)
        self.ax2.text(measured_pos[0], measured_pos[1], '⚽', 
                     ha='center', va='center', fontsize=25)
        
        # Goalkeeper
        self.ax2.plot(goalkeeper_x, goalkeeper_y, 's', markersize=25,
                     color='blue', markeredgecolor='black', markeredgewidth=2)
        self.ax2.text(goalkeeper_x, goalkeeper_y, '🧤', 
                     ha='center', va='center', fontsize=20)
        
        # Arrow showing collapse
        self.ax2.annotate('COLLAPSE!', xy=measured_pos, xytext=(7, 7),
    ...
    def run_experiment(self):
        ...
        plt.savefig(f"{output_dir}/soccer_wavepacket_{timestamp}.png",
