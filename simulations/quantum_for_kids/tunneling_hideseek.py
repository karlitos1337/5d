...
class TunnelingVisualizer:
    ...
    def setup_plot(self):
        ...
        self.fig.suptitle('Quantum Tunneling via VERSTECKSPIEL! 👻🧱', 
    ...
    def draw_barrier(self, ax, particle_pos=None, tunneled=False):
        ...
        # Barrier
        barrier_rect = patches.Rectangle((2, 0), self.tunneling.a, self.tunneling.V0,
        ...
        ax.axhline(self.tunneling.E, color='blue', linestyle='--', linewidth=2,
                  label=f'Particle Energy (E = {self.tunneling.E} eV)')
        
        # Barrier height line
        ax.axhline(self.tunneling.V0, color='red', linestyle='--', linewidth=2,
        ...
        if particle_pos is not None:
            ...
            ax.plot(particle_pos, self.tunneling.E, 'o', markersize=30,
                   color=color, markeredgecolor='black', markeredgewidth=2)
            ax.text(particle_pos, self.tunneling.E, emoji, ha='center', va='center',
                   fontsize=20)
            
            if tunneled:
                # Arrow showing tunneling
                ax.annotate('TUNNELED! 👻', xy=(particle_pos, self.tunneling.E),
                          xytext=(6, 8), arrowprops=dict(arrowstyle='->', lw=3, color='green'),
    ...
    def draw_wave_function(self, ax):
        ...
        # Annotations
        ax.text(0, ax.get_ylim()[1]*0.8, 'Incident Wave\n(Approaching)',
               ha='center', fontsize=9, bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.5))
        ax.text(7, ax.get_ylim()[1]*0.8, 'Transmitted Wave\n(Tunneled!)',
    ...
    def visualize_attempts(self, num_attempts=100):
        ...
        for i in range(num_attempts):
            ...
            if (i + 1) % 10 == 0:
                ...
                self.ax3.axhline(self.tunneling.T, color='r', linestyle='--', linewidth=2,
                ...
                self.ax4.clear()
                self.ax4.axis('off')
                self.ax4.text(0.1, 0.9, 'QUANTUM TUNNELING:', 
                            fontsize=14, fontweight='bold', transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.80, f'Attempts: {i+1}/{num_attempts}',
                            fontsize=12, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.72, f'Successes: {success_count} ({success_count/(i+1)*100:.2f}%)',
                            fontsize=12, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.64, f'Expected: {self.tunneling.T*100:.4f}%',
                            fontsize=12, transform=self.ax4.transAxes)
                
                self.ax4.text(0.1, 0.52, '⚛️ WIE FUNKTIONIERT ES?',
                            fontsize=12, fontweight='bold', transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.44, '1. Teilchen hat zu wenig Energie',
                            fontsize=10, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.38, f'   (E = {self.tunneling.E} < V₀ = {self.tunneling.V0})',
                            fontsize=10, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.30, '2. Klassisch: BLOCKED! 🚫',
                            fontsize=10, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.22, '3. Quantum: TUNNELT durch! 👻',
                            fontsize=10, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.14, f'4. Wahrscheinlichkeit: e^(-2κa)',
                            fontsize=10, transform=self.ax4.transAxes)
                self.ax4.text(0.1, 0.06, '   → Exponentiell klein, aber NICHT NULL!',
                            fontsize=10, transform=self.ax4.transAxes)
    ...
    def run_experiment(self):
        ...
        # Run attempts
        successes = self.visualize_attempts(num_attempts=100)
        ...
        plt.savefig(f"{output_dir}/tunneling_hideseek_{timestamp}.png",
