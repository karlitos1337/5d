...
class QuantumMinecraftVisualizer:
    ...
    def visualize_step(self, x, y):
        ...
        im1 = self.ax1.imshow(superposition_map, cmap='YlOrRd', 
        ...
        for i in range(self.world.size):
            for j in range(self.world.size):
                if not self.world.measured[i, j]:
                    ...
                    text = '❌'
                    color = 'gray'
                self.ax1.text(j, i, text, ha='center', va='center', 
                            fontsize=10, color=color, fontweight='bold')
        
        # Highlight current block
        rect1 = patches.Rectangle((y-0.5, x-0.5), 1, 1, 
        ...
        im2 = self.ax2.imshow(collapsed_map, cmap='Blues', 
                             vmin=0, vmax=1, origin='upper')
            ...
            for j in range(self.world.size):
                if self.world.measured[i, j]:
                    if self.world.blocks[i, j] == 1:
                        ...
                        text = '🪨'
                        color = 'gray'
                    self.ax2.text(j, i, text, ha='center', va='center', 
        ...
        rect2 = patches.Rectangle((y-0.5, x-0.5), 1, 1, 
                                  linewidth=3, edgecolor=result_color, facecolor='none')
        self.ax2.add_patch(rect2)
        
        # Add result text
        self.ax2.text(self.world.size/2, -1.5, result_text, 
    ...
    def run_experiment(self, num_measurements=10):
        ...
        plt.savefig(f"{output_dir}/minecraft_quantum_{timestamp}.png", 
