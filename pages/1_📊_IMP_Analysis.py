        ...
        with open("web/5d-map/data/baseline.json", encoding="utf-8") as f:
            ...
            for code in depression.keys():
                if code in dropout and code in wgi_rl:
                    ...
                    countries[code] = {"imp": round(imp, 3), "depression": dep, "dropout": dro, "governance": gov}
    ...
    with col2:
        ...
        if world_imp:
            ...
            with col_m2:
                ...
                st.metric("Highest", f"{max_country[1]['imp']:.3f}", help=f"Country: {max_country[0]}")

            with col_m3:
                min_country = min(world_imp.items(), key=lambda x: x[1]["imp"])
                st.metric("Lowest", f"{min_country[1]['imp']:.3f}", help=f"Country: {min_country[0]}")
            ...
            st.caption("IMP-Proxy: (1 - Depression/100) × (1 - Dropout/100) × Normalized Governance")
