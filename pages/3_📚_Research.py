...
def create_research_institutions_map(institutions_data):
    ...
    m = folium.Map(
        location=[45, 10], zoom_start=2, tiles="OpenStreetMap", width="100%", height=400
    )
...
def main():
            ...
            if selected_keyword:
                ...
                if arxiv_papers:
                    for i, paper in enumerate(arxiv_papers[:10], 1):
                        with st.expander(
                            f"{i}. {paper.get('title', 'No title')[:80]}..."
                        ):
                            st.markdown(
                            ...
                            if len(paper.get("authors", [])) > 3:
                                st.markdown(
                                    f"*...und {len(paper['authors']) - 3} weitere*"
                                )
                            st.markdown(
    ...
    with col_right:
        ...
        render_minimap(
            m,
            "Academic papers on alternative education by country (arXiv, PubMed, WHO)",
        )
