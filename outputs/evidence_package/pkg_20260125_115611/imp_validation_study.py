...
Ziel: Empirische Validierung der 5 Dimensionen (Pilotstudie, N=150)
"""
...
# Fragebogen-Definitionen (Updated for 5D-Intelligence Framework)
QUESTIONS = {
    "Autonomy": [
        "I feel free to express my ideas and opinions.",
        "I feel like I am the author of my own life.",
        "I have the freedom to make my own choices.",
        "I pursue goals that are meaningful to me.",
        "I feel my decisions reflect my own values.",
    ],
    "Intrinsic_Motivation": [
        "I work on tasks because they truly interest me.",
        "I enjoy challenges without needing external rewards.",
        "I set ambitious goals for myself.",
        "I persist with projects even when they are difficult.",
        "Learning and growth are more important to me than grades or praise.",
    ],
    "Social_Participation": [
        "I actively contribute to group projects.",
        "I can communicate my ideas clearly and persuasively.",
        "I listen attentively to others and build on their ideas.",
        "Collaboration with others enriches my work.",
        "I can empathize well with others' perspectives.",
    ],
    "Resilience": [
        "I quickly recover my performance after setbacks.",
        "I can regulate my emotions even in difficult situations.",
        "Stress affects my performance only temporarily.",
    ...
    "Authenticity": [
        ...
        "I act in a way that is consistent with my values.",
        "I feel comfortable showing my true self.",
        "My actions align with my inner beliefs.",
...
class IMPValidationStudy:
    ...
    def __init__(self):
        self.data = None
        self.results = {}
        self.discriminant_validity = {"passed": False, "high_correlations": []}
    ...
    def _interpret_alpha(self, alpha):
        ...
        if alpha >= 0.9:
            ...
            return "Akzeptabel (Revision empfohlen)"
        elif alpha >= 0.6:
            return "Fragwürdig"
        else:
            return "Inakzeptabel"
    ...
    def correlation_analysis(self):
        ...
        self.check_discriminant_validity(correlation_matrix)

        return correlation_matrix

    def check_discriminant_validity(self, corr_matrix):
        """
        Prüft Diskriminante Validität (Korrelationen sollten < 0.85 sein)
        """
        print("\n=== DISKRIMINANTE VALIDITÄT (Check < 0.85) ===")
        high_corr = []
        ...
        for i in range(len(corr_matrix.columns)):
            for j in range(i + 1, len(corr_matrix.columns)):
                ...
                if abs(val) >= 0.85:
                    ...
                    high_corr.append({"pair": pair, "correlation": float(val)})
                    print(f"⚠️  Hohe Korrelation: {pair} ({val:.3f})")

        if not high_corr:
            print("✅ Diskriminante Validität bestätigt (keine Korrelationen >= 0.85)")
            self.discriminant_validity = {"passed": True, "high_correlations": []}
            return True
        else:
            print(f"❌ {len(high_corr)} Paare mit zu hoher Korrelation gefunden.")
            self.discriminant_validity = {"passed": False, "high_correlations": high_corr}
            return False
    ...
    def visualize_results(self):
        ...
        axes[0, 0].axvline(x=0.8, color="green", linestyle="--", label="Gut-Schwelle (0.8)")
        axes[0, 0].axvline(x=0.7, color="red", linestyle="--", label="Min-Schwelle (0.7)")
        ...
        # Re-run correlation analysis to ensure matrix is available if called independently
        # But usually called in sequence. We can use self.correlation_analysis() which also updates discriminant_validity
    ...
    def _generate_recommendation(self):
        ...
        discrim_valid = self.discriminant_validity["passed"]

        if avg_alpha >= 0.8 and discrim_valid:
            return "Das 5D-Framework zeigt gute bis exzellente Reliabilität und Validität. Bereit für Preprint-Publikation."
        elif avg_alpha >= 0.7:
            return "Das Framework ist akzeptabel, aber Verbesserung einzelner Items oder Trennschärfe empfohlen."
        else:
...
def main():
    ...
    # Increase n to 150 as per protocol
    n_participants = 150
    print(f"\n[2/5] Generiere Beispiel-Daten ({n_participants} Probanden)...")
    np.random.seed(42)
    example_data = {}
    ...
    print(f"Diskriminante Validität: {'BESTÄTIGT' if report['discriminant_validity']['passed'] else 'NICHT BESTÄTIGT'}")

    print("\nNÄCHSTE SCHRITTE:")
    print(f"  1. Echte Probanden rekrutieren (Ziel: {n_participants}+)")
