...
class IMPValidationStudy:
    ...
    def check_discriminant_validity(self):
        """Prüft auf Diskriminante Validität (Korrelation < 0.85)"""
        if not hasattr(self, 'correlation_matrix') or self.correlation_matrix is None:
            self.correlation_matrix = self.correlation_analysis()

        # Maske für Diagonale und unteres Dreieck entfernen (symmetrisch)
        # Wir wollen nur die oberen Korrelationen prüfen, ohne Diagonale (die ist 1.0)
        mask = np.ones(self.correlation_matrix.shape, dtype=bool)
        np.fill_diagonal(mask, 0)

        # Max Korrelation
        # .values.max() ensures we get a scalar from numpy array if it's a DataFrame
        # Wir nehmen den Absolutwert, da starke negative Korrelation auch Diskriminanzprobleme bedeuten
        max_corr = self.correlation_matrix.where(mask).abs().max().max()

        return {
            "valid": bool(max_corr < 0.85),
    ...
    def generate_report(self):
        ...
        discriminant_validity = self.check_discriminant_validity()
        ...
        report = {
            ...
            "recommendation": self._generate_recommendation(discriminant_validity),
        }
    ...
    def _generate_recommendation(self, discriminant_validity=None):
        ...
        if discriminant_validity:
            disc_valid = discriminant_validity["valid"]

        if avg_alpha >= 0.8 and disc_valid:
            return "EMPIRISCH BESTÄTIGT. Das 5D-Framework zeigt gute Reliabilität und Validität. Bereit für Preprint-Publikation."
        elif avg_alpha >= 0.8 and not disc_valid:
            return "REVISION NÖTIG. Gute Reliabilität, aber mangelnde Diskriminanzvalidität (Dimensionen überlappen zu stark)."
        elif avg_alpha >= 0.7:
            return "REVISION EMPFOHLEN. Das Framework ist akzeptabel (α < 0.8), aber Verbesserung der Items nötig."
        else:
            return "KRITISCHE REVISION NÖTIG. Reliabilität unter Schwelle."
...
def main():
    ...
    # Init correlation matrix early for validity check
    study.correlation_matrix = study.correlation_analysis()
