...
class IMPValidationStudy:
    ...
    def check_discriminant_validity(self, threshold=0.85):
        """
        Prüft Diskriminante Validität (Korrelationen < Threshold)
        """
        if self.data is None:
            return False
        ...
        print("\n🔎 DISKRIMINANTE VALIDITÄT CHECK:")
        if high_corr_pairs:
            print(f"⚠️  Warnung: {len(high_corr_pairs)} Paare mit Korrelation >= {threshold} gefunden:")
            for pair, val in high_corr_pairs:
                print(f"  - {pair[0]} <-> {pair[1]}: {val:.3f}")
            return False
        else:
            print(f"✅  Alle Dimensionen sind hinreichend verschieden (Korrelationen < {threshold})")
            return True
...
def main():
    ...
    study.check_discriminant_validity()
