...
class IMPValidationStudy:
    ...
    def generate_report(self):
        ...
        valid_discriminant, high_corrs = self.check_discriminant_validity()
        ...
        report = {
            ...
            "discriminant_validity": {
                "passed": valid_discriminant,
                "high_correlations": high_corrs
            },
            "recommendation": self._generate_recommendation(valid_discriminant),
        }
    ...
    def check_discriminant_validity(self):
        """Prüft Diskriminante Validität (Korrelation < 0.85)"""
        # Wir rufen correlation_analysis auf, aber unterdrücken den Print, da wir es hier prüfen
        dimensions = list(QUESTIONS.keys())
        dim_means = {}
        ...
        return (len(high_corrs) == 0), high_corrs

    def _generate_recommendation(self, valid_discriminant=True):
        ...
        # Reliability Check
        if avg_alpha >= 0.8:
            rec.append("Reliabilität: Gut bis Exzellent (Bereit für Preprint).")
        else:
            rec.append("Reliabilität: < 0.8. Item-Revision empfohlen.")

        # Discriminant Validity Check
        if not valid_discriminant:
            rec.append("Diskriminante Validität: WARNUNG (Korrelationen >= 0.85 gefunden). Dimensionen überlappen zu stark.")
        else:
            rec.append("Diskriminante Validität: Bestätigt (Alle Korrelationen < 0.85).")
