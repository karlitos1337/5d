...
## 2024-06-17 - React Scroll Event Throttling Cache
**Learning:** Even when using passive event listeners, continuously calling `document.getElementById` inside a scroll handler loop causes measurable main-thread blocking.
**Action:** Cache DOM elements corresponding to static sections outside the scroll handler loop so they are only queried once, significantly reducing the overhead of each scroll event tick.
