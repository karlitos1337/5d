...
## 2024-10-24 - Scroll Listener Layout Thrashing
**Learning:** Accessing layout properties like `offsetTop` inside a loop within a scroll event listener forces the browser to recalculate style and layout synchronously for every iteration, causing severe jank.
**Action:** Throttle scroll listeners using `requestAnimationFrame` and cache layout values (like element positions) outside the scroll handler whenever possible.
