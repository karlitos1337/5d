...
## 2025-05-22 - [Unbounded Read in Proxy]
**Vulnerability:** The `owid_proxy.py` read the full response body into memory (`resp.read()`) without size limits, enabling DoS via memory exhaustion. It also lacked `X-Content-Type-Options: nosniff`.
**Learning:** Development proxies are often overlooked for security, but treating them as production code prevents local DoS and information leaks.
**Prevention:** Enforced `MAX_RESPONSE_SIZE` (10MB) with chunked reading and added standard security headers.
