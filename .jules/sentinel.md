...
## 2026-05-16 - [XSS via Third-Party Markdown Parsing]
**Vulnerability:** External HTML pages like `awesome-piracy-main/index.html` parsed unsanitized markdown into `innerHTML`. This exposes the application to DOM-based XSS if the external content contains malicious HTML.
**Learning:** Never trust inputs from external APIs or markdown sources. Always sanitize Markdown output before inserting it into the DOM, especially when using `innerHTML`.
**Prevention:** Imported `DOMPurify` and wrapped `marked.parse(text)` with `DOMPurify.sanitize()` prior to DOM injection.
