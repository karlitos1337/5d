...
## 2025-05-19 - [DOM-based XSS via innerHTML in Static External Pages and Error Handlers]
**Vulnerability:** External HTML sources (`awesome-piracy-main/index.html` and `awesome-ai-web-search-main/index.html`) parsed markdown and directly injected it via `innerHTML` without sanitization. In `web/templates/5d_forschungsplanung.html`, `innerHTML` was used in `catch` blocks and fallback error handlers to display error messages.
**Learning:** Always sanitize external inputs (like markdown or API responses) before inserting them via `innerHTML`. For error handlers, especially inside `catch` blocks where a missing `DOMPurify` import or failure would cause a `ReferenceError` leading to silent failures, prefer replacing `innerHTML` with native DOM element creation (e.g., `document.createElement('p')`) and setting `textContent`. Ensure `DOMPurify` is explicitly imported via `<script>` tag when sanitizing static HTML files.
**Prevention:** Imported `DOMPurify` and used `DOMPurify.sanitize()` in external static HTML files. Replaced `innerHTML` in `catch` blocks and fallback error handlers with native DOM creation and `textContent`.
