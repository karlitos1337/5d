...
## 2025-05-18 - [DOM-based XSS via innerHTML in Error Handlers]
**Vulnerability:** The error handler for the Gemini API call in `web/templates/5d_forschungsplanung.html` used `aiResponse.innerHTML = \`<p class="text-red-500">Netzwerkfehler: \${error.message}</p>\`;`. This is vulnerable to DOM-based XSS if the `error.message` can be manipulated or if an attacker can control the input to the catch block. Moreover, relying on `DOMPurify` inside a try block can lead to silent failures if the library fails to load, causing the `catch` block to execute and directly inject unsanitized content.
**Learning:** Never use `innerHTML` inside error handlers, especially with unescaped template variables. Native DOM methods (`document.createElement` and `textContent`) are inherently safe from XSS as they treat input as text nodes rather than parsed HTML.
**Prevention:** Replaced `innerHTML` assignment with `document.createElement('p')` and `textContent` in the `catch` block. Always prefer native DOM manipulation over `innerHTML` when interpolating dynamic data.
