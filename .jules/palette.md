...
## 2024-04-30 - Accessible Skip Links and Live Regions
**Learning:** In the 5D-map application, while a skip link anchor (`<a href="#main-content" class="skip-link">`) existed, the target `<main id="main-content">` lacked the necessary `tabindex="-1"` and `outline: none;` styles. Without these, focus wouldn't reliably land or stay visibly correct on the main container for keyboard users. Furthermore, dynamic text like the validation count changes dynamically, but screen readers aren't notified unless `aria-live` is present.
**Action:** Always ensure skip link targets (`<main>` or specific `<div>` elements) have `tabindex="-1"` and remove the default focus ring if visually disruptive, so keyboard navigation properly continues from the target without a jarring visual artifact. When dynamic counts or statuses update in the UI without a page load, wrap them in or add `aria-live="polite"` so assistive technologies announce the updates.
