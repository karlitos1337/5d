...
## 2024-05-04 - [Accessibility] Skip Link Target Fix in Vanilla JS Maps
**Learning:** Skip links (`<a href="#main-content">`) in traditional HTML/CSS applications fail if the target `<main>` element lacks `tabindex="-1"` and a CSS rule to suppress default focus outlines. While the page may scroll, keyboard focus doesn't actually transfer, forcing the user to tab through the entire header again.
**Action:** When implementing skip links, always ensure both components are present: the anchor link and the target container configured with `tabindex="-1"` and `outline: none;` (or equivalent) to ensure true programmatic focus transfer.
