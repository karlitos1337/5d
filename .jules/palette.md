...
## 2026-05-15 - Accessible Icon Buttons in Manual Dark Mode
**Learning:** When using manual state for dark mode instead of standard class-based setups, applying template literals to dynamically toggle `focus-visible:ring-offset` classes ensures keyboard focus indicators maintain high contrast across themes.
**Action:** Always test keyboard navigation in both light and dark modes to verify focus ring visibility, and apply conditional offset colors if the background changes significantly.
