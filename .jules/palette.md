...
## 2025-05-11 - [Accessibility] Skip links, focus rings, and icon labels in React
**Learning:** Even though React applications like `web/validation_dashboard` are modern, they frequently miss essential keyboard navigation aids like skip links and dynamic focus-visible rings (especially when using custom dark mode state instead of tailwind's `dark:` classes). Icon-only toggles (like Dark/Light mode and mobile menus) must have `aria-label` and `title`.
**Action:** Implemented a hidden-until-focused skip link directed at `<main id="main-content" tabIndex="-1" ...>`, dynamically adjusted focus rings (`${darkMode ? 'focus-visible:ring-offset-gray-900 focus-visible:ring-blue-400' : 'focus-visible:ring-offset-white focus-visible:ring-blue-600'}`), and added labels to icon-only buttons. Always ensure both `aria-label` and `title` are present for icon buttons.
