...
## 2024-05-24 - [Accessibility] Fix missing target for "skip to main content" link
**Learning:** In the `web/5d-map` project, the "Zum Inhalt springen" skip link was added to `index.html`, but the target `<main id="main-content">` tag was missing `tabindex="-1"` and `outline: none;`. Without these attributes, the main content area cannot receive programmatic focus after jumping, preventing keyboard users from actually skipping the header controls, and leaving an unsightly default focus ring if it did.
**Action:** When adding skip links, always verify that the target container (`<main id="main-content">`) actually has the corresponding `id`, `tabindex="-1"`, and a hidden focus style (`outline: none;` or equivalent) to ensure a complete and polished accessible interaction.
