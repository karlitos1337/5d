...
## 2024-05-14 - Add ARIA Labels for Icon-Only Navigation Buttons
**Learning:** Icon-only toggle buttons (like dark mode or mobile menus) are inaccessible without screen reader context. The `web/validation_dashboard` had React code where `X`, `Menu`, `Moon`, and `Sun` lucide-react icons were purely visual. Also learned about escaping characters like `&apos;` in React for proper HTML rendering.
**Action:** Consistently add `aria-label` tags with descriptive, stateful text (e.g., "Switch to light mode" vs "Switch to dark mode") directly to `<button>` elements, especially for top-level navigation components.
