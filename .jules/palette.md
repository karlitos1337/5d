...
## 2024-05-25 - [Accessibility] Dynamic Focus Offsets and ARIA Labels
**Learning:** When reviewing React single-page applications, it's crucial to ensure icon-only buttons (like dark mode and menu toggles) have descriptive `aria-label` attributes. Additionally, focus-visible states need to explicitly apply dynamic `ring-offset` properties when dark mode is managed by state instead of native class prefixes.
**Action:** Add `aria-label` tags to icon buttons, and implement dynamic focus ring offsets (`focus-visible:ring-offset-*`) for better contrast in both light and dark modes.
