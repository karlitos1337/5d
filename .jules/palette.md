...
## 2025-02-24 - Dynamic focus ring offsets for manual dark mode
**Learning:** In React applications using manual state for dark mode instead of standard class-based setups, static `ring-offset` utilities can lead to poor contrast for keyboard focus rings.
**Action:** Use template literals to conditionally apply the correct `ring-offset` class (e.g., `${darkMode ? 'focus-visible:ring-offset-gray-900' : 'focus-visible:ring-offset-white'}`) based on the manual state.
