...
## 2025-03-09 - Add ARIA labels and focus styles to icon-only buttons
**Learning:** Missing ARIA labels and proper focus styles on icon-only buttons (like dark mode toggles and mobile menus) cause accessibility issues, especially for screen reader users and keyboard navigation. Additionally, ESLint warnings (like unescaped entities or unused variables) can fail the build in strictly configured environments.
**Action:** Always add descriptive `aria-label`s to icon-only buttons, use `aria-expanded` for toggles like menus, and add `focus-visible` styles for better keyboard accessibility. Resolve basic linting issues (like unescaped quotes) to ensure the build passes cleanly.
