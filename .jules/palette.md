...
## 2025-02-28 - Adding ARIA labels to Icon Buttons
**Learning:** Icon-only buttons (like theme toggles or menu icons) are completely invisible to screen readers without ARIA labels, creating a major accessibility barrier. Additionally, focus states are crucial for keyboard navigation.
**Action:** Always include `aria-label` and `title` attributes on icon-only buttons, use `aria-expanded` for toggle menus, and add explicit focus styles (e.g., `focus-visible:ring-2`) to ensure keyboard navigability and screen reader support.
