...
## 2025-05-08 - [Accessibility] Improve keyboard nav for Icon buttons
**Learning:** Icon-only navigation buttons in React dashboards frequently lack aria-labels and clear keyboard focus states, hindering accessibility and navigation for screen readers and keyboard users. Additionally, relying solely on standard focus rings in dark mode components can cause poor contrast.
**Action:** Add `aria-label` tags to icon-only buttons, and utilize dynamic `focus-visible:ring-offset-*` classes (e.g., `${darkMode ? 'focus-visible:ring-offset-gray-900' : 'focus-visible:ring-offset-white'}`) based on state to ensure high contrast visibility for keyboard users.
