...
## 2026-05-20 - ARIA Labels and Keyboard Focus for Icon Buttons
**Learning:** Icon-only buttons (like dark mode toggles or mobile menus) in this application lack context for screen readers and lack visual focus indicators for keyboard navigation, impairing accessibility for users relying on non-pointer inputs.
**Action:** Always verify that icon-only buttons have dynamic `aria-label`/`title` attributes reflecting their current state (e.g. `Zum hellen Modus wechseln`), `aria-expanded` for toggle menus, and `focus-visible:ring-2` for clear keyboard focus states.
