...
## 2024-05-03 - Added tabindex for Skip Link and aria-live to validation counter
**Learning:** When using skip links in a web application (`<a href="#main-content">`), the target container (`<main id="main-content">`) must explicitly receive focus programmatically without causing a visual outline when clicked.
**Action:** Added `tabindex="-1"` and `style="outline: none;"` to the target `<main id="main-content">` to enable proper programmatic focus. Added `aria-live="polite"` and `aria-atomic="true"` to dynamic screen reader announcements.
