...
def place_pulsar(grid: np.ndarray, top: int, left: int) -> None:
    _base = np.zeros((17, 17), dtype=int)  # noqa: F841
    pts = [
        (2, 4),
        (2, 5),
        (2, 6),
...
def place_diehard(grid: np.ndarray, top: int, left: int) -> None:
    pts = [(0, 6), (1, 0), (1, 1), (2, 1), (2, 5), (2, 6), (2, 7)]
    for r, c in pts:
        if 0 <= top + r < grid.shape[0] and 0 <= left + c < grid.shape[1]:
            grid[top + r, left + c] = 1
...
with st.sidebar:
    ...
    with st.expander("Skalierung (Min/Max)"):
        ...
        with c2:
            ...
            b["steps_max"] = st.number_input(
                "Steps max",
                min_value=b["steps_min"],
                max_value=200000,
                value=b["steps_max"],
                step=10,
            )
            b["interval_max"] = st.number_input(
                "Intervall max (ms)",
                min_value=b["interval_min"],
                max_value=60000,
                value=b["interval_max"],
                step=10,
            )
    ...
    size = st.slider(
        "Grid-Größe",
        min_value=int(b["size_min"]),
        max_value=int(b["size_max"]),
        value=default_size,
        step=1,
    ...
    steps = st.slider(
        "Schritte (Auto-Run)",
        min_value=int(b["steps_min"]),
        max_value=int(b["steps_max"]),
        value=min(60, int(b["steps_max"])),
        step=1,
        help="Auto-Run Startschritte: mehr = weiter vorspulen, weniger = feinere Beobachtung.",
    )
    interval_ms = st.slider(
        "Intervall (ms)",
        min_value=int(b["interval_min"]),
        max_value=int(b["interval_max"]),
        value=min(120, int(b["interval_max"])),
        step=10,
        help="Zeit pro Tick: kleiner = flüssiger, größer = ruhiger.",
    )
    size_px = st.number_input(
        "Bild-Pixelgröße",
        min_value=int(b["px_min"]),
        max_value=int(b["px_max"]),
        value=min(400, int(b["px_max"])),
        step=50,
        help="Bildgröße: größer = lesbarer, kleiner = schnelleres Rendering.",
    )
    st.subheader("Auto-Run")
    autoplay_batch = st.selectbox(
        "Batch-Steps pro Tick",
        [1, 2, 5, 10],
        index=0,
...
if save_history_btn:
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    hist = (
        np.array(st.session_state.get("history", []), dtype=int)
...
if export_gif_btn:
    try:
        ...
        st.download_button(
            "GIF herunterladen",
            buf,
            file_name=f"gol_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.gif",
            mime="image/gif",
        )
...
if export_mp4_btn:
    if not HAS_IMAGEIO:
        st.warning(
            "imageio/ffmpeg nicht verfügbar – bitte 'pip install imageio imageio-ffmpeg' und ffmpeg installieren."
        )
    else:
        try:
            frames, h = simulate_frames(
...
with st.expander("Editor-Modus (Zellen anklicken/setzen)"):
    enable_editor = st.checkbox(
        "Editor aktiv",
        value=False,
        help="Bearbeite das Grid als Tabelle (0/1). Für sehr große Grids kann das langsam sein.",
    )
    if enable_editor:
        n = st.session_state.grid.shape[0]
        if n > 50:
