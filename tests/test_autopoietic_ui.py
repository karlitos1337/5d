...
if not st.runtime.exists():
    ...
    st.columns = lambda count: [
        type("Column", (), {"write": lambda *a, **k: None, "markdown": lambda *a, **k: None})
        for _ in range(count)
    ]
    st.tabs = lambda tabs: [
        type("Tab", (), {"write": lambda *a, **k: None, "markdown": lambda *a, **k: None})
        for _ in tabs
    ]
    st.expander = lambda *args, **kwargs: type(
        "Expander", (), {"__enter__": lambda s: s, "__exit__": lambda s, e, t, b: None}
    )()
    st.container = lambda *args, **kwargs: type(
        "Container", (), {"__enter__": lambda s: s, "__exit__": lambda s, e, t, b: None}
    )()
    st.empty = lambda *args, **kwargs: type("Empty", (), {"write": lambda *a, **k: None})()
    st.spinner = lambda *args, **kwargs: type(
        "Spinner", (), {"__enter__": lambda s: s, "__exit__": lambda s, e, t, b: None}
    )()
