...
def test_link_id_present():
    html = read_html()
    assert (
        'id="bootswatch-css"' in html
    ), "bootswatch link id bootswatch-css not found in index.html"
...
def test_whitelist_present():
    ...
    for theme in ("darkly", "united", "flatly", "quartz"):
        assert (
            theme in html
        ), f"expected theme '{theme}' to be present in whitelist in index.html"
...
def test_bootswatch_variables_present():
    ...
    assert (
        "bootswatchVersion" in html
    ), "bootswatchVersion reference not found in index.html"
...
def test_theme_config_centralized():
    ...
    html = read_html()
    assert (
        "THEME_CONFIG" in html
    ), "THEME_CONFIG object not found - configuration should be centralized"
    # Verify the config object contains expected properties
    assert (
        "THEME_CONFIG.bootswatchVersion" in html or "bootswatchVersion:" in html
    ), "bootswatchVersion not in THEME_CONFIG"
    assert (
        "THEME_CONFIG.defaultTheme" in html or "defaultTheme:" in html
    ), "defaultTheme not in THEME_CONFIG"
    assert (
        "THEME_CONFIG.allowedThemes" in html or "allowedThemes:" in html
...
def test_url_building_function_exists():
    ...
    html = read_html()
    assert (
        "buildThemeUrl" in html
    ), "buildThemeUrl function not found - URL construction should be centralized"
...
def test_sanitization_present():
    ...
    assert (
        "[^a-z0-9\\-]" in html or "[^a-z0-9-]" in html
    ...
    assert (
        "/gi," in html or "/gi;" in html or "/gi " in html
...
def test_encoding_present():
    ...
    html = read_html()
    assert (
        "encodeURIComponent" in html
    ), "encodeURIComponent not found - URL components should be encoded"
...
def test_error_handling_present():
    ...
    assert (
        "onerror" in html
    ), "onerror handler not found - should handle CSS load failures"
    assert (
        "onload" in html
    ), "onload handler not found - should verify successful CSS loading"
...
def test_version_validation():
    ...
    assert (
        "safeVersion" in html
    ), "safeVersion function not found - version validation required"
    assert (
        "safeFallbackVersion" in html
    ), "safeFallbackVersion not in config - need pinned safe version"
    # Check for semver-like regex pattern
    assert (
        "semverPattern" in html or "semver" in html.lower()
...
def test_preload_pattern():
    ...
    html = read_html()
    assert (
        "rel = 'preload'" in html or 'rel = "preload"' in html
    ), "preload pattern not found - should use preload for FOUC reduction"
    assert (
        "as = 'style'" in html or 'as = "style"' in html
...
def test_crossorigin_attribute():
    ...
    assert (
        "crossOrigin" in html
    ), "crossOrigin attribute not found - needed for future SRI support"
    assert (
        "'anonymous'" in html or '"anonymous"' in html
